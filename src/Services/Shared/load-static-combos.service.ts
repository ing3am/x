...
import { CitiesService } from '../OldBack/cities.service';
import { LocalService } from '../Config/local.service';
import { CountriesService } from '../OldBack/countries.service';
import { TaxTypeService } from '../OldBack/tax-type.service';
import { StatesService } from '../OldBack/states.service';
import { CiiucodeService } from '../OldBack/ciiucode.service';
import { TermsService } from '../OldBack/terms.service';
import { PaymethodService } from '../OldBack/paymethod.service';
import { CurrenciesService } from '../POS/currencies.service';
import { UomServiceService } from '../OldBack/uom-service.service';
import { TrandoctypeService } from '../OldBack/trandoctype.service';
import { CashEquivalentService } from '../OldBack/cash-equivalent.service';
import { ListmiscService } from '../OldBack/listmisc.service';
import { WarehseService } from '../OldBack/warehse.service';
import { TaxresponsabilityService } from '../OldBack/taxresponsability.service';
import { PlanService } from '../OldBack/plan.service';
import { PaymentsMeansIdService } from '../OldBack/payments-means-id.service';
import { Country } from 'src/theme/shared/Models/country';
import { Terms } from 'src/theme/shared/Models/terms';
import { PayMethod } from 'src/theme/shared/Models/pay-method';
import { TaxType } from 'src/theme/shared/Models/tax-type';
import { TypeID } from 'src/theme/shared/Models/type-id';
...
export class LoadStaticCombosService {
  ...
  getUOM() {
    ...
    this.localStorage.getJsonValue('CIA').then(cia => {
      this.uomService.getByCompany(cia)
        .then((res: any) => {
          if (res !== null) {
            res.forEach(a => {
              if (!a.Inactive)
  ...
  async getPaymentMeansCode() {
    this.paymentmeanscode = [];
    await this.listmiscService.getMiscLists('MedioPagoDIAN')
      .then((res: any) => {
        if (res !== null) {
          res.forEach(a => {
            if (!a.Inactive) {
  ...
  async getPaymentMeansID() {
    this.paymentmeansid = [];
    await this.paymentMeansIdServ.getPaymentMeansID()
      .then((res: any) => {
        if (res !== null) {
